...
- **Story 11.5** — Settings window UI refinements: every slider now shows its live value, a hint line with min/default/max, and a ↺ reset button. Distance unit changed from metres to centimetres. "Test Alert" button disables itself (shows "▶ Testing…") while TTS is playing. "Save" button renamed to "Save and close".
- **Story 11.6** — Camera L/R assignment in settings: users can assign which physical camera index maps to Left and Right from a dropdown in the settings window. Swapping is handled automatically (selecting a value already in use swaps the two sides). `config/settings.py` exposes `left_camera_index` / `right_camera_index` defaults; `CameraManager` reads them at startup.
- **Story 11.7** — Live camera previews in settings with handoff: settings window now shows two 320×240 live preview canvases (L and R, 10 fps). On macOS, the main app releases all camera handles before launching the settings subprocess (`on_before_settings` callback in `TrayApp`), so the subprocess can open its own `cv2.VideoCapture` handles. Tray shows "Status: Degraded" while settings is open; reconnect loop re-acquires cameras automatically when the window closes. ADR-013 documents the handoff pattern.
