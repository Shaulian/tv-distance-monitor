...
def test_open_cameras_one_open():
    ...
    with patch("camera.camera_manager.cv2.VideoCapture", side_effect=caps), \
...
def test_open_cameras_backoff_doubles_on_second_retry():
    ...
    caps = [_closed_cap(), _closed_cap(), _closed_cap(), _closed_cap(), _opened_cap(), _opened_cap()]
    with patch("camera.camera_manager.cv2.VideoCapture", side_effect=caps), \
...
def test_read_frames_returns_both_frames_when_both_open():
    ...
    with patch("camera.camera_manager.cv2.VideoCapture", side_effect=[_opened_cap(frame_l), _opened_cap(frame_r)]), \
         patch("camera.camera_manager.time.monotonic", return_value=0.0), \
         patch("camera.camera_manager.time.sleep"):
        mgr = CameraManager()
        mgr.open_cameras()
...
def test_read_frames_left_drops_returns_none_frame():
    ...
    with patch("camera.camera_manager.cv2.VideoCapture", side_effect=[cap_l, _opened_cap()]), \
         patch("camera.camera_manager.time.monotonic", return_value=0.0), \
         patch("camera.camera_manager.time.sleep"):
        mgr = CameraManager()
        mgr.open_cameras()
...
def test_read_frames_right_drops_returns_frame_none():
    ...
    with patch("camera.camera_manager.cv2.VideoCapture", side_effect=[_opened_cap(), cap_r]), \
         patch("camera.camera_manager.time.monotonic", return_value=0.0), \
         patch("camera.camera_manager.time.sleep"):
        mgr = CameraManager()
        mgr.open_cameras()
...
def test_release_calls_release_on_both_caps():
    ...
    with patch("camera.camera_manager.cv2.VideoCapture", side_effect=[cap0, cap1]), \
         patch("camera.camera_manager.time.monotonic", return_value=0.0), \
         patch("camera.camera_manager.time.sleep"):
        mgr = CameraManager()
        mgr.open_cameras()
